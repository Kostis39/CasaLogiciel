...
class Seances(Resource):
    ...
    def post(self):  # Penser à ajouter de la vérification de validité des données
        ...
        if grimpeur.DateNaissGrimpeur:
            age = annee_courante - grimpeur.DateNaissGrimpeur.year
            if age < 16 and not grimpeur.AccordParental:
                return {
                    "message": "Le grimpeur mineur doit avoir un accord parental"
                }, 403
    ...
    def delete(self, IdSeance):
        with sesh() as session:
            ...
            if not seance:
                return {
                    "message": f"Aucune séance trouvée avec IdSeance {IdSeance}"
                }, 404
...
class SeancesByDate(Resource):  # Ressource pour filtrer les séances par date
    def get(self):
        date_debut = request.args.get(
            "date_debut"
        )  # Si date_fin est fourni, on filtre entre les deux dates sinon c'est la date du jour précisé
