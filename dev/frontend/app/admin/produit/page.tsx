...
export default function ProduitsPage() {
  ...
  // Composant barre verticale d'indentation
      ...
      >
        {Array.from({ length: level }).map((_, i) => (
          <div
            ...
            style={{
              width: 2,
              backgroundColor: '#cbd5e1', // gris clair Tailwind: slate-300
              marginRight: 14,
              height: '100%',
              position: 'relative',
              top: 0,
  ...
  const renderProduit = (id: number, level = 0) => {
    ...
    return (
      <div key={id}>
        <div
          className={`flex items-center space-x-1 py-1 cursor-pointer ${selection === id ? 'bg-blue-100' : ''}`}
          style={{ paddingLeft: 0 }}
          onClick={() => {
            if (isEnsemble) {
              toggleExpand(id);
          ...
          onContextMenu={(e) => {
            e.preventDefault();
            if (selection === id) {
              setSelection(null); // désélectionner si déjà sélectionné
            } else {
              setSelection(id);
            }
          }}
        ...
        >
          ...
          <div className="w-4">
            {isEnsemble ? (
              ...
              <div style={{ width: 16 }} /> // Pour aligner avec les flèches
            )}
          </div>

          <div className="w-4">
            {isEnsemble ? <Folder size={16} /> : <File size={16} />}
          </div>

          <div className="flex-1 flex justify-between pr-2">
            <span>{p.NomProduit}</span>
            {p.PrixProduit !== null && (
              <span className="text-sm text-gray-500">{p.PrixProduit.toFixed(2)} €</span>
            )}
          </div>
        </div>
