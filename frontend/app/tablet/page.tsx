...
export default function DraftForm() {
  ...
  const handleSubmit = form.handleSubmit(async (data: ClientForm) => {
    ...
    try {
      ...
      const message = (err as Error)?.message ?? "Erreur lors de la création du grimpeur";
      toast.error(message);
    } finally {
      setIsSubmitting(false);
    }
    ...
    return (
      <div className="p-2 sm:p-4 md:p-6 lg:p-8 w-full rounded-md gap-2 border overflow-y-auto max-h-screen">
        ...
        >
          ...
          {createdGrimpeurId !== null && (
            <p className="text-center text-sm md:text-base text-muted-foreground mb-6">
              Numéro du grimpeur : <span className="font-semibold text-foreground">{createdGrimpeurId}</span>
            </p>
          )}
  ...
  return (
    <div className="flex flex-col min-h-0 px-2 md:px-4 overflow-y-auto max-h-screen pb-30 ">
      ...
      <div className="flex items-center justify-center mb-6 md:mb-8">
        <div className="flex items-center gap-2 md:gap-4">
          <div className={`flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full border-2 font-semibold transition-colors ${currentStep === 1 ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-400 border-gray-300'
            }`}>
            1
          </div>
          ...
          <div className={`flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full border-2 font-semibold transition-colors ${currentStep === 2 ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-400 border-gray-300'
            }`}>
            2
          </div>
        </div>
